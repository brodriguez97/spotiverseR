#'@title definitions dataframe
#'
#'@description
#'A dataframe containing the Spotify parameters in the Spotify API, their value type (int or float), and their description as it's shown in the Spotify API.
#'
#'@docType data
#'@keywords datasets
#'@name definitions
#'@usage definitons
#'
#'@format A data frame with 13 rows and three variables
#'The variables are as follows:
#'\describe{
#'    \item{KEY}{name of parameter}
#'    \item{VALUE TYPE}{type int or float}
#'    \item{VALUE DESCRIPTION}{description of parameter from Spotify API}
#'}
#'
#'@source The data was sourced from \href{(https://developer.spotify.com/documentation/web-api/reference/object-model/}.
#'
#'
#'@examples
#'head(definitions)


NULL